...
;; Does not work with doom-modeline
(defun mine/evil-operator-state-tag ()
  (let* ((register (or evil-this-register 34))
         (register-contents (evil-get-register register t))
         (register-string (if (null register-contents)
                              (format "%c [Empty]" register)
                            (format "%c: %.10s" register register-contents))))
    (propertize (concat "Register " register-string) 'face '((:background "purple"))))) 
