...
** Setup Package.el to work with MELPA
   ...
   Testing interactive delete
#+begin_src emacs-lisp
(defun foobar-delete (buffer-name)
  (interactive "bThis will delete the buffer")
  (let* ((buffer (get-buffer buffer-name))
         (filename (buffer-file-name buffer)))
    (when buffer
      (when (and filename
                 (file-exists-p filename))
        (message "You chose to delete `%s" (buffer-name))
...
;(package-refresh-contents) ; Uncomment for first run at least
...
(use-package evil
  ...
  (evil-mode)
  :config
  ;; remove if this takes precedence over exiting current emacs commands
  (define-key evil-insert-state-map (kbd "C-g") 'evil-normal-state)
  ;; When in a narrow buffer, visual-line mode splits lines
  ;; This will keep evil from skipping beyond new-line
  (evil-global-set-key 'motion "j" 'evil-next-visual-line)
  (evil-global-set-key 'motion "k" 'evil-previous-visual-line)
  ;; was just to help show the current register
  ;; (setq evil-operator-state-tag 'mine/evil-operator-state-tag)
  )
...
Visual representation of changes, very useful when fat-fingering

Default behavior for ~display-buffer~ attempts to even out window sizes.
Just in case it is useful elsewhere I set it to =height-only= instead of =nil.=
I don't think I need to put this in a hook method, because I don't think the variable will be resized without my permission.
...
We have used ~evil-global-set-key~ earlier for visual-line-mode navigation.
For this particular mode, we do not want that behavior.
It results in ignoring the tree structure and negates the benefit of evil-mode.

+Resize hook borrowed from [[https://stackoverflow.com/questions/68230947/how-to-resize-the-undo-tree-visualizer-window][StackOverflow]]+
...
(defun undo-tree-hook ()
  "Hook to restore proper up/down motion now that we made j/k use visual-line movement by default"
  (interactive)
  (evil-local-set-key 'motion "j" 'evil-next-line)
  (evil-local-set-key 'motion "k" 'evil-previous-line))
...
(nvmap :prefix "SPC"
       ...
       "y l"   '(org-store-link :which-key "Org store link") ; store a link, for use with insert/open/etc
       "m h"   '(org-toggle-heading :which-key "Org toggle heading")
       "m i"   '(org-toggle-item :which-key "Org toggle item")
       "m f"   '(org-narrow-to-subtree :which-key "Org focus")
       "m b"   '(widen :which-key "Org focus back") ; todo write function that toggles instead
       "m B"   '(org-babel-tangle :which-key "Org babel tangle")
       "o a"   '(org-agenda :which-key "Org agenda")
       "o e"   '(eshell :which-key "Open eshell")
       "o g"   '((lambda () (interactive) (find-file "~/org/goals.org")) :which-key "My Goals")
       "o h"   '((lambda () (interactive) (find-file "~/org/habits.org")) :which-key "My Org Habits")
       "o c"   '((lambda () (interactive) (find-file "~/.emacs.d/config.org")) :which-key "Emacs Config")
       "o x"   '(mine/load-perspectives :which-key "Saved Perspectives")
       "o l"   '((lambda () (interactive) (find-file "~/metacog/20220902161514-leveling_system.org")) :which-key "Leveling System")
       "o d"   '(mine/load-project-agenda :which-key "Do Something")
       ...
       "w r"   '(windresize :which-key "Resize Windows")
       "c"     'org-capture
       "t"     'mine/triage-refile-task
       "/"     'swiper
       "g d"   'counsel-etags-find-tag-at-point
       ;; I like the idea of using a prefix for searching, currently swiper prefix could expand to 
       ;; C-c / which is org-sparse-tree (with regex) allows searching down.
       )
#+END_SRC

#+begin_src emacs-lisp
;; This [currently] closes all buffers and loads my default perspective
;; TODO kill buffers only when necessary
(defun mine/load-perspectives ()
  (interactive)
  (mapc 'kill-buffer (buffer-list)) ; perhaps unnecessary
  (persp-state-load "~/.emacs.d/saved-perspectives"))


(setq mine/project-list
      (list
       (cons "Personal" "~/org/agenda.org")
       (cons "Client: PD" "~/org/work.pd.org")
       (cons "Syntonic" "~/org/work.syntonic.org")
       (cons "Family & Friends" "~/org/personal.family.org")
       (cons "Coursera" "~/org/personal.coursera.org")
       (cons "Cooking" "~/org/personal.cooking.org")
       (cons "Whitney" "~/org/personal.whitney.org")
       (cons "House" "~/org/personal.house.org")
       (cons "Piano" "~/org/personal.piano.org")
       (cons "RV Living" "~/org/personal.rv.org")
       (cons "Motorcycle" "~/org/personal.motorcycle.org")
       (cons "Lastpass" "~/org/personal.lastpass.org")
       (cons "Skating" "~/org/personal.skating.org"))
      )


;; Should allow me to expand my GTD workflow nicely
(defun mine/load-project-agenda ()
  (interactive)
  (let ((prompt "Open GTD Project: "))
    (ivy-read prompt mine/project-list
	      :initial-input nil
	      :action (lambda (project-pair) (interactive) (find-file (cdr project-pair))))))


(defun mine/triage-refile-task ()
  (interactive)
  (let ((prompt "Refile to project GTD Project: "))
    (ivy-read prompt mine/project-list
	      :initial-input nil
	      :action (lambda (project-pair) (interactive)
			(ivy-read prompt (list "Q1" "Q2" "Q3" "Q4")
				  :initial-input "Q"
				  :action (lambda (priority) (interactive)
					    (let* ((target-buffer (save-excursion
								    (find-file (cdr project-pair))
								    (current-buffer)))
						   (pos (save-excursion
							  (org-find-exact-headline-in-buffer priority
											     target-buffer))))
					      (org-refile nil nil 
							  (list priority (cdr project-pair) nil pos)
							  "Triaged")
...
(setq org-agenda-custom-commands
      '(("g" . "GTD contexts")
	("gx" "Custom: IMPORTANT & URGENT"
         ((org-ql-block '(and (parent (heading "Q1"))
			      (parent (parent (heading "Action Items"))))
                        ((org-ql-block-header "Important and Urgent")))
          (agenda)))
	("gi" "Custom: IMPORTANT ONLY"
	 ((org-ql-block '(and (parent (heading "Q2"))
			      (parent (parent (heading "Action Items"))))
			((org-ql-block-header "Important")))
          (agenda)))
	("gu" "Custom: URGENT"
	 ((org-ql-block '(and (parent (or (heading "Q1")
					  (heading "Q3")))
			      (parent (parent (heading "Action Items"))))
			((org-ql-block-header "All Urgent Items")))
          (agenda)))))
#+end_src
...
#+BEGIN_SRC emacs-lisp
(nvmap :states '(normal visual) :prefix "SPC"
...
This may be useful to avoid spurious buffer creation, but the package contains much more
than this.  Also, I'll need to investigate the source as it is a Wiki-only package.
...
;; to better edit 80-char style code
;; Not as intense as darkroom / zen
...
| eval-print-last-sexp | /Insert results: elisp exp. before point/      | SPC e p    |
...
(nvmap :states '(normal visual) :prefix "SPC"
       ...
       ;; "e p"   '(eval-print-last-sexp :which-key "Eval +PRINT+ last sexression")
       "e p"   '(pp-eval-last-sexp :which-key "Eval +PRINT+ last sexression")
       "e r"   '(eval-region :which-key "Eval region"))
#+END_SRC

* EVALUATE SLIME (COMMON-LISP) EXPRESSIONS
  I have to admit this is pretty cool. I cannot seem to remember the evaluate bindings.

| COMMAND         | DESCRIPTION                                       | KEYBINDING |
|-----------------+---------------------------------------------------+------------|
| eval-buffer     | TODO /Evaluate lisp in buffer/                      | SPC s b    |
| eval-defun      | TODO /Evaluate the defun containing or after point/ | SPC s d    |
| eval-expression | TODO /Evaluate an lisp expression/                  | SPC s e    |
| eval-last-sexp  | /Evaluate lisp expression before point/             | SPC s l    |
| eval-region     | TODO /Evaluate lisp in region/                      | SPC s r    |

#+BEGIN_SRC emacs-lisp
;; TODO map commented lines to correct functions
(nvmap :states '(normal visual) :prefix "SPC"
       "s s"   '(slime :which-key "Start SLIME session")
       "s x"   '(slime-restart-inferior-lisp :which-key "Restart SLIME session")
       "s b"   '(slime-eval-buffer :which-key "Eval buffer in SLIME")
       "s p"   '(slime-sync-package-and-default-directory :which-key "Sync SLIME package, directory")
       ;"s d"   '(eval-defun :which-key "Eval defun")
       ;"s e"   '(eval-expression :which-key "Eval elisp expression")
       "s l"   '(slime-eval-last-expression :which-key "Eval last s-exp")
       ;"s r"   '(eval-region :which-key "Eval region"))
)
#+END_SRC
...
#+begin_src emacs-lisp
;; (interactivie "r") is for when the mark is always active
;; the commands below were failing on buffer load / emacs startup

;; default behavior uses first line of region only
;; The commented lines
;; allow multi-line search strings joined by " "
;; does not work well for code, and introduces
;; the problem of paragraph or file-long selections
(defun mine/find-file-rg (&optional start end)
  (interactive (when (use-region-p)
		 (list (region-beginning) (region-end))))
  (setq deactivate-mark t)
  ;; (counsel-rg (when start
  ;; 		(downcase (string-join (mapcar 'string-trim-left (split-string (buffer-substring start end) "\n")) " ")))
  (counsel-rg (when (use-region-p)
		(downcase (string-trim-left (buffer-substring start end))))
              nil nil nil))

(defun mine/find-file-rg-case-sensitive (&optional start end)
  (interactive (when (use-region-p)
		 (list (region-beginning) (region-end))))
  (setq deactivate-mark t)
  (counsel-rg (when (use-region-p)
                (string-trim-left (buffer-substring start end)))
              nil nil nil))
#+end_src
...
(nvmap :states '(normal visual) :prefix "SPC"
       ...
       "f g"   '(mine/find-file-rg :which-key "Find file using rg")
       "f G"   '(mine/find-file-rg-case-sensitive :which-key "Find file using rg (Case Sensitive)")
...
;; I also found this snippet for async:
;; https://emacs.stackexchange.com/questions/19075/regenerate-ctags-tags-file-without-freezing-emacs-for-a-while
...
;; Testing improvement for Reddit user
;; Just shows the current register for operation in modeline
(defun mine/doom-modeline-register-info ()
  (let* ((register (or evil-this-register 34))
	 (register-contents (evil-get-register register t))
	 (register-label (if (bound-and-true-p mine/doom-modeline-register-info--simple)
			     (format "r[%c]" register)
			   (format "[reg %c]: %.10s" register register-contents)))
	 (register-string (if (null register-contents) "[Empty]" register-contents)))
    (propertize register-label
		'face 'doom-modeline-highlight
		'help-echo (format "Register %c : %s" register register-contents))))

(defun mine/doom-modeline-inject-register (modeline-def)
  (cons (append (car modeline-def)
		(list '(:eval (doom-modeline-segment--register-info))))
	(cdr modeline-def)))
...
(use-package doom-modeline
  ...
  :config
  (setq mine/doom-modeline-register-info--simple t)
  (doom-modeline-def-segment register-info
    (concat (doom-modeline-spc)
	    (mine/doom-modeline-register-info)))

  (advice-add 'doom-modeline-format--main :filter-return #'mine/doom-modeline-inject-register)
  )

(advice-add 'evil-use-register :before (lambda (_) (force-mode-line-update)))
...
* AVY
Avy movements are like Vimium / EasyMotion.  It has evil-integration /but/ I'm not ready yet.
...
I want autocomplete like I used to have in my IDE days.
We are loading completion-at-point-functions, dabbrev (local-file) and etags

I was working forever to get this working in SLIME.
My current hypothesis is that slime-company loads /from/ the SLIME *​inferior-lisp* buffer.  Originally I assumed that it was fundamental to getting the mode working at all.

I found ~company-files~ for filename completion in the future.

#+begin_src emacs-lisp
(use-package company
  :config
  ;; I removed slime-company from this because I had a dependency loop
  ;; and also I don't know if it even makes sense.
...
Ok, here goes: this will require access via MS.  Are they still using GPT-3?

#+begin_src emacs-lisp

;; Dependencies (already installed: dash, s)
...
;; I refuse to use straight.el or quelpa at this point
;; I want to understand my package system, worry about monkey patching
(add-to-list 'load-path "~/source/repos/copilot.el")
(require 'copilot)
...
;; removed C-s binding in favor of leader key
;; This way I can use avy for convenient navigation
(use-package swiper
  :after ivy)
#+end_src
...
** Evil Org Mode
Trying this on for size.  This will replace heading/subtree movement (e.g, instead of C-c C-f for forward, use g modifier)

#+begin_src emacs-lisp
(use-package evil-org
  :after org
  ; I feel like this should hook itself...
  :hook (org-mode . (lambda () (evil-org-mode)))
  :config
  ;; (require 'evil-org-agenda)
  ;; (evil-org-agenda-set-keys) ; maybe remove...
  )
#+end_src

** Capture Template for Triage (GTD)

#+begin_src emacs-lisp
;; TODO, make this general.  Leverage a data model somewhere that specifies project status(es)
(setq org-capture-templates
      '(("t" "Triage" entry (file+headline "~/org/agenda.org" "Triage")
         "** %?\n")
	("s" "Triage Syntonic" entry (file+headline "~/org/work.syntonic.org" "Triage")
         "** %?\n")
	("h" "Triage House" entry (file+headline "~/org/personal.house.org" "Triage")
         "** %?\n")
	("p" "Triage PD" entry (file+headline "~/org/work.pd.org" "Triage")
         "** %?\n"))) 
#+end_src
...
It is imperative that the ~texlive~, ~texlive-fonts-extra~ and ~texlive-latex-extra~ packages are installed.
Also note that viewing PDF required both ~pdf-tools~ and =M-x pdf-tools-install=

#+begin_src emacs-lisp
;; Why two methods of customizing vars?
...
This is a problem I've faced for a long time.  Sometimes I want to look at what's beneath fontify.

#+begin_src emacs-lisp

;; We may want to extend org-appear
;; the problem is that we want to support non-org files
;; currently requires 'evil
(defun mine/raw ()
  (interactive)
  (save-excursion
    (unwind-protect
	(progn
	  (with-silent-modifications
	    (let* ((inner-WORD (unless (use-region-p)
				 (evil-select-inner-object 'evil-WORD nil nil nil 1)))
		   (start (if (use-region-p) (region-beginning) (car inner-WORD)))
		   (end (if (use-region-p) (region-end) (cadr inner-WORD)))
		   (my-overlay (make-overlay start end))
		   (contents (buffer-substring start end))
		   )
	      (when (> (length (string-trim contents)) 0)
		(set-text-properties 0 (length contents) '(face highlight) contents)
		(overlay-put my-overlay 'display contents)
		(sit-for 0.25)
		(set-text-properties 0 (length contents) '(face region) contents)
		(sit-for 3)
		))))
      (deactivate-mark)
      (remove-overlays)))
...
;; This needs a mode-toggle
(use-package org-appear
  ;; we need to unbind C-; in undo-tree/config
  :init
...
(plain-list-item . nil))))
#+END_SRC
** Clean Folded Outlines
Do *not* fold blank lines into sub-headings!!!
#+begin_src emacs-lisp
...
I realize this isn't the /best/ place for this, but this is where I have the most trouble.  My beautiful perspectives get messed up by ~undo-tree~ or ~org-agenda~.  This is a separate undo manager for window configurations.  To go back, use ~C-c left~
...
* PERSPECTIVE
Perspective.el lets you manage groups of buffers and window layouts.  Supports save/restore
...
(use-package perspective
  ...
  :init
  (persp-mode)
  (setq persp-state-default-file "~/.emacs.d/saved-perspectives"))
#+end_src
* MAGIT
...
;; Use evil-mode to jump to start of line
(defun my-evil-eshell-fix () 
  (interactive)
  (eshell-bol)
  (evil-insert-state))
...
(add-hook 'eshell-mode-hook 
          (lambda () 
            (evil-define-key 'normal 'eshell-mode-map (kbd "I") 'my-evil-eshell-fix)))
#+END_SRC

** VTerm

VTerm is a full terminal emulator, using external compiled C code.  Unlike Eshell, Term, etc, it can handle just about anything a terminal can.
...
** SLIME
   ...
   This was my original setup and config.  First I installed sbcl, then quicklisp.
   Then I loaded quicklisp to run ~(ql:quickload "quicklisp-slime-helper")~ 
   Then I put this in my config and it was at least /working./
...
For no good reason I wanted ~use-package~ everywhere.  So I cobbled this together from a bunch of different sources.  At some point I have multiple SLIME versions, and started over.  The first time I set this up, I needed to add slime-company manually.  Here it's commented out.

I /do/ believe I wanted to use "slime-company" as a backend for company-mode so that I could auto-populate from things in my Lisp image that were defined in the REPL.  I no longer have that, and it may not even make sense.
...
;; I hate copying code, but this came straight from abo-abo
